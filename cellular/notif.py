    ...
    try:
        ...
        if (registered):
            ...
            # ping command would be at index 5 since it is the last of the 6 commands put in the dictionary by setResponses
                ...
                captureImage = True
                sendFTP(img_dir + "maxframe_90.jpeg", "maxframe_90.jpeg")
        ...
        # not a registered user so sends something about how to register
            ...
            ser.write(
                ('You are not a registerd device. Please send unique registration code provided with device.').encode(
                    'ascii'))
            ser.write('\x1A'.encode('ascii'))
...
def sendFTP(FPATH, FNAME):
    ...
    myTime = str(datetime.datetime.now(pytz.timezone('US/Eastern')))
    # want to send user a txt that image was captured
    send_at("AT+CMGS=\"" + myNum + '\"', ">", 2)
    # if asking to write a message is sucessful we will encode the desired message for the SMSw
    ser.write((r'Image captured at ' + myTime
               + r' Image is processing and will be available shortly at http://73.198.62.223/' + FNAME).encode(
        "ascii"))
    ser.write('\x1A'.encode('ascii'))
    time.sleep(.01)
    send_at(' ', 'OK', .5)
   ...
   # rts2, out2 = send_at('at+cftpput=\"' + FNAME + '\"', '+cftpput: begin', 3.5)
   # time.sleep(.1)
   # n = 256
    # my for loop kludge that keeps me up at night
   # for i in range(0, len(word), n):
    #    print("SARAH BREAKPOINT 2")
    ...
    try:
        ...
        print('failed to clear the buffers')
    return myTime
def run():
...
while (True):
    ...
    time.sleep(8)
    if i<95:
        sendFTP(img_dir+"maxframe_63.jpeg","maxframe_"+i+".jpeg")
        i=i+1
#run()
