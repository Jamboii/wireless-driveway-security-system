...
def send_at(command,back,timeout):
    ...
    if ser.inWaiting():
        ...
        decode_rec_buff = rec_buff.decode('ascii')
        print (decode_rec_buff)
    if back not in decode_rec_buff:
        print('^^^')
        return 0,decode_rec_buff
    else:
        return 1,decode_rec_buff
...
def sendFTP(FPATH,FNAME):
   ...
   # send_at('at+cftpslogin=\"73.198.62.223\",21,\"test\",\"4doi98uyg43ew#\",0','+cftpslogin: 0',8)
    ...
    ser.write(('Image captured at '+myTime).encode("ascii"))
    ser.write(('\x1A').encode('ascii'))
    time.sleep(1)
    #send_at(' ','OK',.5)
    ...
    send_at("AT+CMGS=\""+myNum+"\"",">",1)
    ser.write((r'View image from '+myTime+ 'at http' ).encode('ascii'))
    time.sleep(.01)
    ser.write(('\x1A').encode('ascii'))
    return myTime
